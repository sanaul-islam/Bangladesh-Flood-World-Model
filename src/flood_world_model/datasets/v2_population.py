...
class V2PopulationDataset(Dataset):
    def __init__(
        ...
        if self.static.shape != (
            len(STATIC_VARIABLES),
            self.static.shape[1],
            self.static.shape[2],
        ):
            raise RuntimeError(
                f"Unexpected static shape: {self.static.shape}"
            )

        if self.static.shape[0] != 12:
            raise RuntimeError(
                f"Expected 12 static channels, got {self.static.shape[0]}"
            )
        ...
        # IMPORTANT:
        # A 7-day sample beginning at target_index
        # requires target_index ... target_index+6.
        # Therefore the final valid target start is:
        #
        # end_index - horizon
        #
        # and np.arange upper bound is end_index-horizon+1.
        last_index = (
            end_index
            - horizon
            + 1
        ...
        if first_index >= last_index:
            raise RuntimeError(
                "No valid samples available for this split."
            )
        ...
        if max_required_index >= end_index:
            raise RuntimeError(
                "Dataset produced a sample whose target window crosses the split boundary."
            )
